 #!/usr/bin/env python3
  ...
  def ai_analyze(articles):
      ...
      prompt = f"""你是氢燃料电池汽车产业情报分析师。为项目团队撰写每日情报简报，聚焦全球政策和产业动向。

  今日全球新闻（{len(articles)}条）：
  {articles_text}

  严格按以下结构输出，每条约 50-100 字，Markdown 格式：

  ## 一、政策动向

  按国家列出今日**实质性政策变化**。每条写明：国家/机构、政策名称或措施、具体内容（补贴金额/产量目标/技术标准/时间节点）
  。**区分"新发布"与"已有政策的延续"**，关键数字用 **加粗** 标注。末尾用 `→[利好/收紧/观望]` 标注趋势判断。

  ## 二、产业要闻

  今日重大产业动态：企业战略调整、产能建设、示范项目、投融资、供应链变化。写明：企业名、事件详情、涉及金额或产能规模。对
  国内有借鉴意义的标注 `【可对标】`。

  ## 三、技术前沿

  今日关键技术进展：效率提升、成本下降、材料突破、耐久性进展。写明：机构名、突破指标、当前阶段（实验室/中试/量产）。

  只输出三个部分的内容。不要开头语、结束语、免责声明。⚠️必须逐一分析上方列出的所有新闻条目，确保不遗漏重要信息。"""
      ...
      result = http_post_json(AI_ENDPOINT, {
          ...
          "messages": [
              {"role": "system", "content":
  "你是资深燃料电池产业分析师。输出精炼、专业、只说实质内容。不要客套话、不要免责声明、不要开头和结尾语。"},
