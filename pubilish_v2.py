...
def get_tenant_token():
    ...
    _tenant_token = resp["tenant_access_token"]
    _tenant_token_expiry = time_module.time() + resp.get("expire", 7200)
    return _tenant_token
    ...
    for attempt in range(3):
        try:
            ...
            if result.get("code") == 0:
                return True
            print("  [WARN] Feishu API: " + str(result.get("msg", result)))
            if attempt < 2: time_module.sleep(2)
        except Exception as e:
            print("  [WARN] send_card attempt " + str(attempt+1) + ": " + str(e))
            if attempt < 2: time_module.sleep(2)
    return False
# ── Card building ──────────────────────────────────────────────────
