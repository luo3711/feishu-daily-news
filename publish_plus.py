...
A self-contained, config-driven tool that:
  ...
  3. Pushes formatted interactive cards via Feishu enterprise self-built bot.
...
--- Secrets (env vars or .env) ---
  ...
  FEISHU_APP_ID         Feishu enterprise self-built app ID      (required)
  FEISHU_APP_SECRET     Feishu enterprise self-built app secret  (required)
  FEISHU_OPEN_ID        Target user open_id to send messages to  (required)
...
# ===================================================================
# Feishu enterprise bot delivery
# ===================================================================
...
def _get_tenant_access_token(app_id: str, app_secret: str) -> str:
    """Obtain (or return cached) tenant_access_token from Feishu."""
    now = _time.time()
    if _token_cache.get("token") and _token_cache.get("expires_at", 0) > now + 60:
        return _token_cache["token"]

    resp = http_post_json(_FEISHU_AUTH_URL, {
        "app_id": app_id,
...
def send_card(
    ...
    """Push one interactive card via Feishu enterprise bot (3 retries)."""
    card: Dict[str, Any] = {
        "header": {
    ...
    for attempt in range(3):
        ...
        url = f"{_FEISHU_MSG_URL}?receive_id_type=open_id"
        req = Request(
            url, data=body,
            headers={
        ...
        try:
            ...
            if result.get("code") == 0:
                return True
            logger.warning(
                "  Feishu API returned code=%s msg=%s",
    ...
    for a in articles:
        ...
        for name, regions in buckets_def.items():
            if a["region"] in regions:
                ...
                placed = True
                break
        if not placed and "\u5176\u4ed6" in buckets:  # "Other"
            buckets["\u5176\u4ed6"].append(a)
...
class Delivery:
    ...
    def __init__(self, mode: str, output_file: Optional[Path] = None,
                 app_id: str = "", app_secret: str = "", open_id: str = "") -> None:
        self.mode = mode
    ...
    def push(self, title: str, content: str, color: str = "blue") -> bool:
        ...
        # feishu mode
        ok = send_card(title, content, color, self.app_id, self.app_secret, self.open_id)
...
Return ONLY the JSON object. No preamble, no markdown fences, no closing remarks.""", encoding="utf-8")
    ...
    # ---- .env.example ----
    (HERE / ".env.example").write_text("""# Feishu enterprise self-built app credentials (required)
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    open_id = _require_env("FEISHU_OPEN_ID")
